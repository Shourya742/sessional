...
for i=1:length(t)
    ani=subplot(1,1,1);
    L1_bar=line([A(1) B(1,i)],[A(2) B(2,i)], 'Color', 'blue', 'linewidth', 2);
    L2_bar=line([B(1,i) C(1,i)],[B(2,i) C(2,i)], 'Color', [0.5 0 0.5 0.8], 'linewidth', 2);
    L3_bar=line([C(1,i) D(1)],[C(2,i) D(2)], 'Color', 'c', 'linewidth', 2);
    L_bar_fixed=line([E_down(1,i) E_up(1,i)],[E_down(2,i) E_up(2,i)], 'Color', [0.8 1 0 0.25], 'linewidth', 5);
...
set(vel,'XLim',[0 5],'YLim',[0 40]);
xlabel(vel, 'Time (s)');
ylabel(vel,'Amplitude (m/s)');
title(vel,'Speed of E');
grid on;

        acc = subplot(3,1,2);
        plot(acc,t(1:i),ang_acc_D(1:i));
        set(acc,'XLim',[0 5],'YLim',[-40 40]);
        xlabel(acc, 'Time (s)');
        ylabel(acc, 'Angular Accelaration (rad/s)');
        title(acc, 'Angular Accelaration (Input Angular Acc. = 0)');
        grid on;
        
torque=subplot(3,1,3);
plot(torque,t(1:i),A_torque(1:i));
set(torque,'XLim',[0 5],'YLim',[0 100]);
xlabel(torque, 'Time (s)');
ylabel(torque,'Torque (N-s)');
title(torque,'Torque Required at Crank A');
grid on;